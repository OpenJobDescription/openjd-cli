...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            id="With all optional arguments (-tp)",
        ),
        pytest.param(
            [
                "some-template.json",
            ...
            id="With all optional arguments (--tasks)",
        ),
        pytest.param(
            [
                "some-template.json",
            ...
            id="With all optional arguments (--maximum-tasks)",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ["run", "somefile.json", "-tp", "Foo=Bar", "--tasks", '[{"Foo": "Bar"}]'],
            id="-tp/--tasks mutually exclusive",
        ),
        pytest.param(
            ["run", "somefile.json", "-tp", "Foo=Bar", "--maximum-tasks", "1"],
            id="-tp/--maximum-tasks mutually exclusive",
        ),
        pytest.param(
            ["run", "somefile.json", "--tasks", '[{"Foo": "Bar"}]', "--maximum-tasks", "1"],
            id="--tasks/--maximum-tasks mutually exclusive",
        ),
    ],
)
